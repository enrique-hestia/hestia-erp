...
// ── CacheService helper ─────────────────────────────────────────
// Envuelve una función de lectura con un caché de script de GAS.
// TTL máx recomendado: 300 s (5 min) para datos que mutan con frecuencia.
// Silently falls back to no-cache if payload > 100 KB.
function _cachedResponse(key, ttlSeconds, fn) {
  ...
  try { cache.put(key, json, ttlSeconds); } catch(e) {}
...
function doGet(e) {
  try {
    ...
    if (action === 'ingresos') {
      return _cachedResponse('gas_ingresos_v1', 300, readIngresosData);
    }

    if (action === 'productos') {
      return _cachedResponse('gas_productos_v1', 600, readProductos);
    }
    ...
    if (action === 'egresos') {
      ...
      var _egAnio = anioEg ? parseInt(anioEg) : new Date().getFullYear();
      return _cachedResponse('gas_egresos_v1_' + _egAnio, 300, function(){ return readEgresosData(_egAnio); });
    }

    if (action === 'listaPacientes') {
      return _cachedResponse('gas_pacientes_v1', 600, listaPacientesAll);
    }
